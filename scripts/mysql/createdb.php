...
if($vars["newdbname"] == "") {
	Header("Location: dbmanager.php?msg=".urlencode(_CREATEDBCHOOSE_));
	exit();
}

/* perform a query to insert the data into the db */
if(!@mysql_create_db($vars["newdbname"])){
	Header("Location: error.php?errmsg=".urlencode(@mysql_errno()." : ".@mysql_error()." : "._CREATEDBERROR_));
	exit();
}
...
Header('Location: dbmanager.php?&msg='.urlencode(_CREATEDBGOOD_));
