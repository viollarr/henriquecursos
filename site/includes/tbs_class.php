...
function DataPrepare(&$SrcId,&$oTBS) {
	...
	if ($this->Type===7) {
		...
		if (function_exists($this->FctOpen)) {
			...
			$this->Type = $this->DataAlert('The data source Id \''.$Key.'\' is an unsupported '.$SubKey.'. And the corresponding custom function \''.$this->FctClose.'\' is not found.');
		}
	}
...
function GetBlockSource($BlockName,$List=false) {
	...
	$Nbr = 0;
	$Pos = 0;
	while ($Loc = $this->meth_Locator_FindBlockNext($this->Source,$BlockName,$Pos,'.',false,$P1)) {
		$P1 = false;
		$Nbr++;
...
// This position can be useful because we don't know in advance how $Value will be replaced.
	...
	if (isset($Loc->PrmLst['onformat'])) {
		if ($Loc->FirstMerge) {
			$Loc->FctInfo = $Loc->PrmLst['onformat'];
			$Loc->FctPrm = array(&$Loc->FullName,'',&$Loc->PrmLst,&$this);
			$ErrMsg = false;
			if (!$this->meth_Misc_UserFctCheck($Loc->FctInfo,$ErrMsg)) {
				unset($Loc->PrmLst['onformat']);
				if (!isset($Loc->PrmLst['noerr'])) $this->meth_Misc_Alert('Parameter \'onformat\'',$ErrMsg);
				$Loc->FctInfo = 'pi'; // Execute the function pi() just to avoid extra error messages 
			}
		}
		$Loc->FctPrm[1] = &$CurrVal;
		if (isset($Loc->PrmLst['subtpl'])) {
			$this->meth_Misc_ChangeMode(true,$Loc,$CurrVal,true,true);
			call_user_func_array($Loc->FctInfo,$Loc->FctPrm);
	...
	if ($Script) {// Include external PHP script
		...
		if (isset($Loc->PrmLst['once'])) {include_once($File);} else {include($File);}
...
// Return a locator object covering all block definitions, even if there is no block definition found.
	...
	do {
		...
		$Loc = $this->meth_Locator_FindBlockNext($Txt,$BlockName,$Pos,'.',false,$LocR->P1);

		if ($Loc===false) {

			if ($Pid>0) {
				// Disconnect $Txt from the source
				$Src = $Txt;
				$Txt = &$Parent[$Pid]['txt'];
				if ($LocR->BlockFound) {
	...
	if ($LocR->BlockFound===false) {
		...
		tbs_Locator_SectionAddBlk($LocR,$BlockName,$Txt);
		$Sid = 1;
		$LocR->SectionBid[1] = $Bid;
		$LocR->SectionIsSerial[1] = false;
		$LocR->NoDataBid = $Bid;
	} elseif ($LocR->WhenFound and ($Sid===0)) {
	...
	while ($BlockId<$BlockNbr) {
		...
		if ($LocR->BlockFound) {
			...
			if (isset($LocR->PrmLst['onsection'])) {
				$LocR->FctInfo = $LocR->PrmLst['onsection'];
				$ErrMsg = false;
				if ($this->meth_Misc_UserFctCheck($LocR->FctInfo,$ErrMsg)) {
					$LocR->FctPrm = array($BlockName,'','','');
					$NoFct = false;
				} else {
		...
		if ($QueryOk) {
			if ($Src->Type===4) { // Special for Text merge
				if ($LocR->BlockFound) {
					...
					$Src->CurrRec = false;
					if ($NoFct===false) {
						$LocR->FctPrm[1] = &$Src->CurrRec ; $LocR->FctPrm[2] = &$Src->RecSet; $LocR->FctPrm[3] = &$Src->RecNum;
						call_user_func_array($LocR->FctInfo,$LocR->FctPrm);
					}
				...
				while($Src->CurrRec!==false) {
					...
					if ($GrpFound) {
						...
						if ($LocR->FooterFound) {
							...
							for ($i=$LocR->FooterNbr;$i>=1;$i--) {
								...
								if ($Src->RecNum===1) {
									...
									if ($change_i) {
										$grp_change = true;
										$grp_src = $this->meth_Merge_SectionNormal($LocR,$LocR->FooterBid[$i],$PrevRec,$PrevNum,$PrevKey,$AvoidFct).$grp_src;
										$LocR->FooterPrevValue[$i] = $x;
									}
								}
							}
						...
						if ($LocR->HeaderFound) {
							...
							for ($i=1;$i<=$LocR->HeaderNbr;$i++) {
								...
								if ($change) {
									$grp_src .= $this->meth_Merge_SectionNormal($LocR,$LocR->HeaderBid[$i],$Src->CurrRec,$Src->RecNum,$Src->RecKey,$AvoidFct);
					...
					if ($SerialMode) { // Serial Merge
						...
						if ($LocR->WhenFound) { // With conditional blocks
							...
							$i = 1;
							do {
								$cond = $this->meth_Merge_SectionNormal($LocR,$LocR->WhenCondBid[$i],$Src->CurrRec,$Src->RecNum,$Src->RecKey,$AvoidFct);
								if (tbs_Misc_CheckCondition($cond)) {
									$x_when = $this->meth_Merge_SectionNormal($LocR,$LocR->WhenSectionBid[$i],$Src->CurrRec,$Src->RecNum,$Src->RecKey,$AvoidFct);
							...
							if (($found===false) and ($LocR->WhenDefaultBid!==false)) {
								$x_when = $this->meth_Merge_SectionNormal($LocR,$LocR->WhenDefaultBid,$Src->CurrRec,$Src->RecNum,$Src->RecKey,$AvoidFct);
				...
				if ($LocR->FooterFound) {
					if ($Src->RecNum>0) {
						for ($i=1;$i<=$LocR->FooterNbr;$i++) {
							if ($LocR->FooterIsFooter[$i]) $BlockRes .= $this->meth_Merge_SectionNormal($LocR,$LocR->FooterBid[$i],$PrevRec,$PrevNum,$PrevKey,$AvoidFct);
						}
					}
				}
...
function meth_Merge_SectionNormal(&$LocR,&$BlockId,&$CurrRec,&$RecNum,&$RecKey,&$NoFct) {
	...
	if ($NoFct) {
		...
		$Txt0 = $Txt;
		$LocR->FctPrm[1] = &$CurrRec ; $LocR->FctPrm[2] = &$Txt; $LocR->FctPrm[3] = &$RecNum;
		call_user_func_array($LocR->FctInfo,$LocR->FctPrm);
		if ($Txt0===$Txt) {
			$LocLst = &$LocR->BlockLoc[$BlockId];
			$iMax = $LocLst[0];
...
// onload - onshow
	...
	while ($LocA=$this->meth_Locator_FindBlockNext($Txt,$Name,$Pos,$ChrSub,true,$P1)) {

		// Check for self embeded files
		if ($LocA->PosEnd>$PosMax) {
			$PosMax = $LocA->PosEnd;
			$NbrSelf = 0;
		} else {
			$NbrSelf++;
			if ($NbrSelf>64) return $this->meth_Misc_Alert('Automatic fields','The field '.$this->ChrOpen.$LocA->FullName.$this->ChrClose.' can\'t be merged because the limit (64) is riched. You maybe have self-included templates.');
		}
