...
***********************************************/
	...
	<div class="subMenu" style="display:inline;">
		...
		elseif ($c == 1){
			...
			if(($x['nome']) == 'FISIOT. EMPRESARIAL ERGONOMICA'){
				echo "<div>";
				echo "<option value='cal_ergonomia.php?id=".$x[id]."' style='color:$color;'>$x[nome]</option>'";
				echo "</div>\n";
				$a = ($a) + 1;
			}
			elseif(($x['nome'] != 'FISIOT. EMPRESARIAL ERGONOMICA BÁSICA') and ($x['nome'] != 'FISIOT. EMPRESARIAL ERGONOMICA AVANÇADA')){
				echo "<div>";
...
<script type="text/javascript">
 ...
 dhtmlselect("webmaster")
		...
		<select name="galeria" class="menu" style=""  size="2" id="galeria" title="&nbsp;GALERIA" onchange='if(options[selectedIndex].value) top.location.href= (options[selectedIndex].value)'>
        	<div>
        	<option value='galeria.php' style='color:$color;'>FOTOS</option>
            <option value='videos.php' style='color:$color;'>VIDEOS</option>
            </div>
        </select>
			<script type="text/javascript">
         		//dhtmlselect("id_of_select_menu", "optional_width_of_select_box_px", "optional_width_of_drop_down_menu_px")
